...
def train_model(feedback_df):
    ...
    # KNN - finds songs most similar to liked songs based on audio features
...
def get_recommendations(df, feedback_df, knn, scaler, top_n=10):
    heard_ids = set(feedback_df['track_id'].tolist())
    unseen = df[~df['track_id'].isin(heard_ids)].copy()

    X_scaled = scaler.transform(unseen[AUDIO_FEATURES].values)
    unseen['score'] = knn.predict_proba(X_scaled)[:, 1].clip(0, 1)
...
def main():
    ...
    if not feedback_df.empty and len(feedback_df) >= MIN_SONGS_TO_TRAIN:
        ...
        if likes > 0 and dislikes > 0:
            print("Loading model...")
            knn, scaler = train_model(feedback_df)
            model_active = True
            print("Top picks for you:")
            top5 = get_recommendations(df, feedback_df, knn, scaler, top_n=20)
            shown = 0
            for _, s in top5.iterrows():
                if shown >= 5:
                    break
                if not all(ord(c) < 128 for c in s['track_name'] + s['artists']):
                    continue
                shown += 1
                print(f"  {shown}. {s['track_name']} - {s['artists'].split(';')[0]} ({s['track_genre']})")
            print()
    ...
    try:
        while True:
            ...
            if model_active and not explore:
                recs = get_recommendations(df, feedback_df, knn, scaler, top_n=50)
                if recs.empty:
